...
- **Database (Google Sheets)**:
  ...
  - **Kapazität (Data Sheet)**: The live database where capacity ratings ("Noch gut Luft", "Passt", "Zu voll") are **appended (never updated)**. Multiple rows per (date, slot) are allowed; the latest row is the current value. The override dialog still appears to the user when an entry already exists for a slot, but confirming it appends a new row rather than mutating the old one. Each row carries an `Eingetragen am` column (ISO 8601 local datetime, `YYYY-MM-DDTHH:MM:SS`) recording the wall-clock submission time, distinct from `Datum` which is the training date the entry refers to.
- **Data Fetching Strategy**:
  ...
  - **Submit refresh** (`fetch_recent_capacity`): On submission, only the Kapazität tab is re-fetched (2 API calls) to check for concurrent writes that would trigger the override dialog. Stammdaten and Kürzel are already cached and not re-read.
...
- **State Management & Caching**:
  ...
  - Sheet data is stored in `st.session_state`:
    ...
    - `_cached_data_df`: recent Kapazität rows for the overview. After a successful submit (including confirmed overrides), the new row is **appended** into this DataFrame via `pd.concat` — it is never set to `None` after a submit, so the overview reloads instantly without a spinner. It is only set to `None` as a fallback when a cache update cannot be performed.
...
- **Concurrent Write Detection**: During form submission, `fetch_recent_capacity` fetches fresh data. If an entry for that specific slot/day was created by another user in the interim, the app prompts an override dialog (`confirm_override_dialog`) which calls `submit_entry` to append a new row. After a confirmed override, the new row is appended into `_cached_data_df` via `pd.concat`; `find_existing_entry` returns `iloc[-1]` so the overview immediately reflects the new value.
- **Append-Only History (UX-hidden)**: Every confirmed submission — including overrides — appends a new row. The user is not made aware of this; the overview/dialog continue to present a single 'current' value per slot. Consumers deduplicate to the latest row per (Datum, slot) via `iloc[-1]` in `find_existing_entry` for the overview, and via `sort_values("Eingetragen am") + drop_duplicates(keep="last")` in `render_statistics`.
